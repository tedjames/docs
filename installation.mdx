...
1.  **Download the Chrome Extension**: The official chrome extension is pending approval on the Google extension marketplace. For now, you can [click here to download the extension](https://github.com/AgentDeskAI/browser-tools-mcp/releases/download/v1.1.0/chrome-extension-v1-1-0.zip) or clone the repo below and find the extension within the chrome-extension directory:
...
<Accordion title='Error: Screenshot tool is failing'>
    ...
    Cursor and some other MCP clients may not always fetch the correct version of the MCP server. Make sure to use `npx @agentdeskai/browser-tools-mcp@1.1.0` instead to explicitly use the latest working version.
